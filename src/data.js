export const projects = [
  {
    title: 'Teastation ',
    subtitle: 'CSS and Javascript',
    description:
      'This is a wonderful site where you can order tea. Great setup and use of CSS and Javascript ',
    image: './project-1.jpg',
    link: 'https://teastationproject2.netlify.app',
  },
  {
    title: 'Cocktails ',
    subtitle: 'React',
    description:
      'With this site you can choose what cocktails you like, also in the details it will show you what is in the drink.',
    image: './project-2.gif',
    link: 'https://react-project1-cocktails.netlify.app/',
  },
  {
    title: 'Github user',
    subtitle: 'React ',
    description:
      'This site you can search for Github users and check out their github sites.',
    image: './project-3.gif',
    link: 'https://project4-githubuser-search.netlify.app',
  },
  {
    title: 'Travel App',
    subtitle: 'Javascript and CSS',
    description:
      'A travel application that can assist you in finish wonderful holidays.',
    image: './project-4.gif',
    link: 'https://backroadsproject.netlify.app',
  },
  {
    title: 'Comfy Store',
    subtitle: 'Javascript and CSS',
    description:
      'A store app that you can choose and be happy with new furniture. Was a fun project to accomplish.',
    image: './project-5.gif',
    link: 'https://project3confystore.netlify.app/',
  },

  {
    title: 'Tin Dog',
    subtitle: 'Javascript and CSS',
    description: 'This is an app like facebook.',
    image: './project-6.gif',
    link: 'https://tindoggrimm.netlify.app/',
  },
  {
    title: 'Comfy Store - React App',
    subtitle: 'React ',
    description:
      'This is an app so you can buy furniture.Also used stripe and Auth0',
    image: './project-7.gif',
    link: 'https://reactcomfyslothstore-project.netlify.app/',
  },
]

export const testimonials = [
  {
    quote:
      'The guys i put in here is not real was just testing out.So comment as much as you like',
    image: 'https://randomuser.me/api/portraits/men/1.jpg',
    name: 'Jesse Hicks',
    company: 'Zoozle',
  },
  {
    quote: 'Time will come when the portfolio will do great things for me',
    image: 'https://randomuser.me/api/portraits/men/94.jpg',
    name: 'Ruben Alvarez',
    company: 'Dooble.io',
  },
]

export const skills = [
  'JavaScript',
  'React.js',
  'CSS',
  'Node',
  'GraphQL',
  'HTML',
]
